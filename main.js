...
}).then((position) => ({ lat: position.latitude, lon: position.longitude }));

const setWeatherForMyLocation = async () => {
  const position = await getPosition();
  const data = await buildData({ coord: position }, APIkey);
  setCurrentWeather(data);
};
